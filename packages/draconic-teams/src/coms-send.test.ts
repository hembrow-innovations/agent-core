...
test("sendComsPrompt does not report a silent endpoint as sent", async () => {
	const dir = mkdtempSync(join(tmpdir(), "draconic-teams-coms-dead-"));
...
test("sendComsPrompt fails on a missing endpoint", async () => {
	const dir = mkdtempSync(join(tmpdir(), "draconic-teams-coms-miss-"));
...
test("sendComsPrompt fails when the peer nacks", async () => {
	const dir = mkdtempSync(join(tmpdir(), "draconic-teams-coms-nack-"));
