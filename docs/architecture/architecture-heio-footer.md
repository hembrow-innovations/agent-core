---
id: "architecture-heio-footer"
title: "One-line TUI footer"
kind: architecture
description: "heio-footer paints one dim status line in TUI mode."
domain: pack
area: architecture
...
`@agentic-core/heio-footer` replaces the TUI footer with one dim line. The layout lives in `formatFooterLine`. Edit that function to change the line.
...
```ts
// packages/heio-footer/src/format.ts formatFooterLine
const parts = [
  fields.cwd,
