...
```ts
// packages/heio-coms/src/index.ts — default export
...
`packages/heio-teams/src/index.test.ts` test `teams does not re-register coms identity flags` asserts the two factories share no flag names.
...
```ts
// packages/heio-coms/src/index.ts — session_start
...
```ts
// packages/heio-coms/src/protocol.ts — paths
...
```ts
// packages/heio-coms/src/index.ts — before_agent_start
const identity = peer
...
```ts
// packages/heio-coms/src/index.ts — coms_list
...
Vendor dest is `.pi/vendor/@agentic-core/heio-coms`. Pack layout is [[architecture-pack-and-packages]]. Terms are [[glossary]].
