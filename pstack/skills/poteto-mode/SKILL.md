...
When running under **OpenCode** (not Cursor), map tooling as follows:

| Cursor / pstack wording | OpenCode equivalent |
|-------------------------|---------------------|
| `Task` tool / `subagent_type: "poteto-agent"` | Task tool with `subagent_type: "poteto-agent"` (agent file `.opencode/agent/poteto-agent.md`) |
| `subagent_type: "Comment Sicko"` | `subagent_type: "comment-sicko"` |
| `generalPurpose` | `general` or `explore` |
| Slash skill `/foo` | Command `/foo` (`.opencode/command/foo.md`) **or** Skill tool `name=foo` |
| Sticky poteto-mode | Primary agent `poteto` + `/poteto-mode` command; stay in that agent |
| `~/.cursor/rules/pstack-models.mdc` | `.opencode/rules/pstack-models.md` (+ optional AGENTS.md notes) |
| Graphite / cursor-team-kit deslop | Skip or approximate with `/unslop`; no Graphite assumed unless the project uses it |
| Worktrees / iOS simulators | Optional; follow project git layout |

**Skill load:** Use the Skill tool to load leaf principles and situational skills by folder name (e.g. `principle-prove-it-works`, `tdd`, `interrogate`).

**Verify:** Prefer a project-local `verify-*` skill when present. Otherwise use the project's documented harness (tests, headless run, e2e). Compile-only is not prove-it-works.

**Subagent depth:** If swarm/arena fan-out fails on nesting, raise `subagent_depth` in `opencode.json`.
