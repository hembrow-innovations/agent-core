...
impact: HIGH
impactDescription: Illegal states compile
tags: [arch, types]
---

## Type System Discipline

**Incorrect:** `{ completed: boolean; completedAt?: Date }` or a stringly `engine_id`.
**Correct:** Make the illegal combination unrepresentable. Brand primitives. Parse at the boundary.
...
- **Derive types from authoritative schemas.** When a protocol buffer, OpenAPI spec, GraphQL schema, database migration, or design-system token file defines a shape, derive from it instead of hand-rolling a parallel type. Manual duplication drifts. See `principle-encode-lessons-in-structure`.
