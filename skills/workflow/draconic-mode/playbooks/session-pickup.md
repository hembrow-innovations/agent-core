...
1. Locate the prior trail. A local OpenCode session under `~/.local/share/opencode/` for this cwd, the current OpenCode session, `/resume` or `/tree`, or a pushed branch. Read the last messages first, then scan back for the decision points. Parse a long transcript with the Task tool or a summary file and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
