...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript under `~/.pi/agent/sessions/` for this cwd, or `PI_SESSION_FILE`. Also check `.heio/sessions/` when that tree exists. Do not glob across unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its rule file, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
