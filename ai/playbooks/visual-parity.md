...
4. Verify each component against its baseline via image diff on the matching surface via the project `verify-*` skill. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Stay in this session or poll with bash per component until the diff is zero.
