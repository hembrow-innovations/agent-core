...
Pi has no Cursor cloud agents, no `environment: "cloud"`, and no dashboard. Local subagent or in-process only. Do one unit per session, prove it, open a PR with `gh`. Do not pretend a cloud fleet ran. Use `gt` only if the repo already uses Graphite. Writers are `subagent` as agent worker with `worktree` true. Read-only reviews are scout or reviewer.
...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. Agents are spawned, resumed, and drained only through `subagent`. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Skip on Pi. One coordinator chat. Nested cloud tracks do not exist. A track this chat can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking nested coordinator hides its children while the parent idles.
- **Worker / verifier.** Local only. A writer is `subagent` as agent worker with `worktree` true. A verifier is scout or reviewer, on a different model family from its worker when that is configured. Prove with the project `verify-*` skill. Transcripts live under `~/.pi/agent/sessions/` for this cwd, or `PI_SESSION_FILE`, and `.heio/sessions/` when that tree exists. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). One unit this session.
...
Ask in prose, only for product or preference. Park the gate first.
...
FORBIDDEN    no gt unless the repo already uses it, no rebase, no force-push, no fixes outside scope, plus unit-specific bans
...
A dependency is a context relay, not just ordering: undeclared upstream context makes the worker guess. Missing fields are a refuse-to-spawn condition. Audit the worker brief for this session's unit. Never resume-chain a brief; respawn fresh with consolidated scope.
...
3. **Pilot.** Push one unit through the whole path: brief, worker, verification, stack entry, ledger row, merge. The pilot exists to falsify the brief template, the verify recipe, and the unit size while that costs one agent instead of fifty. Fix the contract from pilot evidence before any later session. On programs of near-identical cheap units, the first unit is the pilot, run as a normal unit with its verify command inline.
4. **Scale across sessions.** One unit per session. Recompute ready work after each drain; relay upstream reports into the next brief. Do not spawn a rolling cloud window. The sampled brief audit runs with the unit it samples.
5. **Drain.** Run the queue discipline below at every drain point.
6. **Land.** Landing is continuous, never a terminal phase: integration starts with the first verified unit and runs alongside later sessions. The coordinator lands verified units itself per Roles, with `gh`. Keep the frontier green before upper-stack work; Stack safety governs. Advance `frontier.json` only on merge or reported new head SHAs.
7. **Close.** Drain the final inbox, reconcile every spawned agent to a terminal row (done, abandoned, zombie-reconciled), confirm the predicate on the real artifact, confirm every landed PR has a verdict for its current head SHA, audit the trail including a cross-model review, encode recurring corrections into `preferences.md` or the brief template. Leave the store intact; it is the postmortem.
...
- Each drain classifies every pointer (landed, needs-verify, failed, zombie, noise), writes the resulting rows through `orch unit add`, `orch unit set`, and `orch ledger record`, runs `orch status`, then prepares the next session's unit in one message.
- Account for every spawned child at rollup: arrived, respawned, or its scope explicitly absorbed. Silently redoing a missing child's work hides both the wasted spend and the coverage gap its result existed to close.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from git and `gh` after every merge and stack mutation. If the repo already uses Graphite, recompute from `gt` because GitHub base refs drift mid-restack while gt tracking is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it where gt knows the stack, normally the stacker's clone; a checkout whose gt metadata never saw the submits reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may run `gt`, and only if the repo already uses it; record the holder in the standing orders. Restacks are local.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier (scout or reviewer, different model family than the worker) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier whose entire product would be rerunning one command is ceremony, not verification. Prove on the project `verify-*` skill when the change has a real surface.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run: a worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one session when that session dies was never done.

#### Liveness and failure

- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the local session under `~/.pi/agent/sessions/` or `PI_SESSION_FILE`. Transcript mtime is not liveness. There is no Cursor dashboard and no cloud-agent status.
...
- After a Pi or session restart: local agents are dead. There is no cloud remainder. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work by PR and branch rather than agent id, spawn the next unit from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.

#### Escalation

Reaches the human, batched into the status page rather than per item: irreversible actions (force-push to shared branches, deploys, deletions, closing someone else's PR), genuine product or preference calls no experiment settles, a standing order that contradicts observed reality, a program-level dead end that survived a replan. Park each as a `gates.md` entry before asking in prose, and route work around it.
