...
2. Write `purpose.md` (job and fences). Copy `templates/purpose.md`.
3. Write or edit `contract.md` promises. Copy `templates/contract.md`. See `contract-anatomy`.
4. Write `test.md` mapping the tests that cover those promises. Copy `templates/test.md`.
5. Point Authority and `index.md` at the new notes.

Fill frontmatter from `templates/required-fields.md`. Set `id` to the filename stem. Set the h1 to the same string as `title`. Create the parent directory on first write. Do not scaffold empty folders.
