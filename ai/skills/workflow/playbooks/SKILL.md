...
**Prefer:** one playbook per task. Copy its steps into the todolist verbatim. A skip stays as `skip: <reason>`. Dest steps live at `.opencode/playbooks/<id>.md`.
...
Library path here: `ai/playbooks/<id>.md`. Dest: `.opencode/playbooks/<id>.md`.
