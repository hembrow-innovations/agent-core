...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. Agents are spawned and drained only through `subagent`. Writers are agent `worker` with `worktree` true. Read-only is `scout` or `reviewer`. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Skip on Pi. One unit per session leaves nothing for a nested coordinator to drain. If a later session truly needs a track owner, it is still local and in-process.
- **Worker / verifier.** Local or in-process only. Cloud `environment` does not exist. A writer is `worker` with `worktree` true. A verifier is `scout` or `reviewer`. Prefer one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker when both run. Transcripts live under `~/.pi/agent/sessions/` for this cwd, or `PI_SESSION_FILE`, or `.heio/sessions/`.

Depth stays at coordinator then worker. Author the track decomposition per project (build, landing, and verification are common cuts, not a required shape); hard-coded swarm trees were tried and parked as too rigid.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe a product or preference ask.
- `decisions.tsv` is the decision trail.
...
VERIFY       exact commands or the project verify-* skill, plus known gotchas
...
A dependency is a context relay, not just ordering: undeclared upstream context makes the worker guess. Missing fields are a refuse-to-spawn condition. Never resume-chain a brief; respawn fresh with consolidated scope.

#### Steps

1. **Frame.** State the done predicate as something countable ("all 126 units merged, each ledger-verified `unit-test-verified` or better"). Quantify scope: units, rough effort, expected stacks, and the wall-clock budget. If one agent could finish inside that budget, stop here and run Autonomous run instead. Collapsing must not depend on another document being present: it means do the work directly in this session, plain workers where they help, verification inline, landing as you go, and none of the store, register, or pilot machinery below. This session executes one unit. Remaining units stay in `units.tsv` for later sessions. Name the tracks per project. A contested decomposition or one-way door goes through the arena skill before the pilot. Present the framing once; reversible prep proceeds without waiting.
2. **Install the runtime.** Run `orch init`. Open the decision trail, write the standing orders before any spawn, and seed `frontier.json` from existing PRs with `orch frontier set --repo <repo-dir>`.
3. **Pilot.** This session's one unit. Push it through the whole path: brief, worker or in-process, verification, ledger row, PR with `gh`. The pilot exists to falsify the brief template, the verify recipe, and the unit size while that costs one agent instead of fifty. Fix the contract from pilot evidence before any later session fans out.
4. **Scale.** Do not fan out a fleet. Record remaining ready units. The next session picks up the next unit. Do not pretend parallel owners ran.
5. **Drain.** Run the queue discipline below at every drain point.
6. **Land.** Land this session's verified unit with `gh`. Graphite / `gt` only if the project already uses it. Keep the frontier green before upper-stack work; Stack safety governs. Advance `frontier.json` only on merge or reported new head SHAs.
7. **Close.** Drain the inbox for this unit, reconcile it to a terminal row (done, abandoned, zombie-reconciled), confirm the predicate progress on the real artifact, confirm the landed PR has a verdict for its current head SHA, encode recurring corrections into `preferences.md` or the brief template. Leave the store intact; it is the postmortem and the next session's start.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (stay in this session, or poll with bash, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Each drain classifies every pointer (landed, needs-verify, failed, zombie, noise), writes the resulting rows through `orch unit add`, `orch unit set`, and `orch ledger record`, runs `orch status`, then stops. Do not spawn a next wave this session.
- Account for this session's child: arrived, respawned, or its scope explicitly absorbed. Silently redoing a missing child's work hides both the wasted spend and the coverage gap its result existed to close.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` after every merge and stack mutation. Default is `gh`. Use `gt` only if the project already uses it, because GitHub base refs drift mid-restack while gt tracking is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR.
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run locally when the project uses Graphite. There is no cloud restack.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier (`scout` or `reviewer`, on a different model family than the worker) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier whose entire product would be rerunning one command is ceremony, not verification.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run: a worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in this session when the session dies was never done.

#### Liveness and failure

- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches. There is no dashboard. Transcript mtime is not liveness.
...
- After a Pi or session restart: local agents are dead. There is no cloud remainder. Re-read the standing orders and `units.tsv`, recompute the frontier, continue the next unit from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.

#### Escalation

Reaches the human, batched into the status page rather than per item: irreversible actions (force-push to shared branches, deploys, deletions, closing someone else's PR), genuine product or preference calls no experiment settles, a standing order that contradicts observed reality, a program-level dead end that survived a replan. Park each as a `gates.md` entry before asking in prose, only for product or preference, and route work around it.
...
**Reply:** at checkpoints and close: the predicate and the count against it from `units.tsv` and `ledger.tsv`, this session's unit and what it landed, the frontier (PR list plus SHAs), verdicts summary, what was abandoned and why, gates awaiting the human (the only asks), remaining units parked for later sessions, the store path, and the trail path. Numbers from the tables, not narrative. Include PR links.
