...
1. Locate the prior trail. A local session under `~/.pi/agent/sessions/` for this cwd, or `.heio/sessions/`, or `PI_SESSION_FILE`, or a pushed branch. Read the last messages first, then scan back for the decision points. Parse a long transcript with a read-only `subagent` (`scout` or `reviewer`) or a summary file and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
