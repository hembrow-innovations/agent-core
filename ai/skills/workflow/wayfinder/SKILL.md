...
Do not open a GitHub Issue. Do not write the map into `docs/`. Do not invent a folder or a kind under `.heio/`.

## Store

Load **management** before any write under `.heio/`. Load **docs** only when a settled decision should survive a clone. Load **domain-modeling** when a term or ADR belongs in the vault.
...
The map is one plan at `.heio/planning/plans/plan-<N>-<slug>.md`. Tag it `wayfinder`. Status `draft` while charting, `active` while tickets are open, `complete` when the way is clear.
...
- **prototype** (HITL). Load **prototype**. Link the artifact from the issue. Keep throwaway code out of `docs/` and out of `.heio/`.
...
Then load **management**. Set the plan `complete` and move it to `.heio/closed/`.
