---
name: obsidian-axi
description: Obsidian vault CLI (`npx -y @andershoffmann/obsidian-axi`). Use when searching a vault, following wikilinks or backlinks, listing tags, or patching a heading or frontmatter field. Not docs kinds or templates. Not heio-stack issues or plans.
metadata:
  version: "1.0.0"
---
...
Filesystem CLI for an Obsidian vault. TOON output. No plugin, no server, no running app. Per-rule detail lives in `rules/<prefix>-*.md`. Command catalog lives in [AGENTS.md](AGENTS.md) (reference only).
