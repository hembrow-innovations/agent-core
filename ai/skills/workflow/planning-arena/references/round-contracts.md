...
Do not rewrite earlier rounds.

## Questions file

`round-<N>-questions.md`
...
One file per round. One or more questions. `N` in the heading is the question number in that round, starting at 1.

Omit the parent's recommended answer.
...
`round-<N>-questions-<panel_member_name>.md`
...
Judge waits until every answer file exists. Read the questions file and every answer file. Append `#### Selected` under each question on the questions file. Do not rewrite the question text. Do not edit answer files.
...
If spawn is missing, the parent writes the questions file, three answer files, and the selected blocks, and marks `skip: no spawn runtime` on the round. Do not invent child transcripts.
