...
1. Spawn `subagents` with `readonly: true` and tell the child to read `/skill:comment-sicko` then report kills for the scope. If spawn is missing, read `.pi/skills/comment-sicko/SKILL.md` and apply it yourself.
...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The **principals** rules `principle-fix-root-causes` and `principle-redesign-from-first-principles` guide intent only: fix real causes, redesign as if requirements always existed, never bolt on symptom guards. Neither authorizes widening the fence nor fixing instances outside it.
