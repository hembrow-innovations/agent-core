---
name: setup-heio
description: Configure which models heio uses per role. Detects available models and writes .pi/heio-models.md. Use for /setup-heio or changing heio model choices.
---

# Setup heio

Write the model map to `.pi/heio-models.md`. Skills fall back to inherit-parent when a line is absent.
...
Overwrite `.pi/heio-models.md` so re-runs stay idempotent:

```
---
description: heio per-role model choices (overrides skill defaults)
---
