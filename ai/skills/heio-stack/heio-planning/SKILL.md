...
6. Stop. Summarize spec + `EXPECT:` lines. Wait for confirm.
7. Copy `templates/slice-spec.md` and `templates/slice-oracles.md` into the slice folder. Status `frozen`. `EXPECT:` is frozen. The first `CHECK:` is drafted here; the builder may refine the command later.
8. Leave `tasks.md` unwritten. Tasks exist when the slice is `frozen` or `active`, via **heio-slice**.
...
3. Same rule every time:
   - Fits an unblocked active slice → **TASK**. Status `promoted`. If `tasks.md` exists, append the task line. If it does not, name the line for **heio-tasker**.
