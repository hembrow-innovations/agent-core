...
- **TASK**: it fits an unblocked active slice. Do it now, or write it onto that slice's `tasks.md`.
- **TICKET**: it belongs to the project, not this slice. File it under `.heio/tickets/` and leave the slice alone.
- **ESCALATE**: a workflow would rewrite a location destination. Stop. Bump it to the map with the human.
