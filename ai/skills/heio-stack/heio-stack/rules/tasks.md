...
Frozen or active slices may have `tasks.md`. Copy `templates/slice-tasks.md` when the slice turns `frozen` or `active`. **heio-tasker** writes that file from the frozen spec + oracles. Incoming product work becomes a ticket first; triage may append a task.
