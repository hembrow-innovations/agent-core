...
const comsDir = mkdtempSync("/tmp/heio-coms-");
...
async function start(
	name: string,
	extra?: { purpose?: string; cwd?: string; model?: string },
) {
...
test("exports a factory", () => {
	assert.equal(typeof comsExtension, "function");
	comsExtension({
		registerFlag() {},
		registerTool() {},
		on() {},
...
test("two in-process peers talk over the unix socket protocol", async () => {
	...
	assert.equal(
		existsSync(join(comsDir, "projects", "default", "agents", "ghost.json")),
		true,
	);
	const afterPrune = await planner.list();
	assert.equal(
		afterPrune.some((peer) => peer.name === "ghost"),
		false,
	);
	assert.equal(
		existsSync(join(comsDir, "projects", "default", "agents", "ghost.json")),
