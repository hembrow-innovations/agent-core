...
- **Heio-stack**: the tracker. Skills under `ai/skills/heio-stack/`, `heio-*` agents, `@agentic-core/heio-coord`. This checkout runs it. A dest only gets it if the profile lists it. The `management` skill is a different `.heio/` convention; it is not this checkout's tracker.
...
- **Session extensions**: `packages/heio-boot`, `heio-footer`, `heio-coord`, `heio-onic`. In-session Pi plugins. Not hivemind.
