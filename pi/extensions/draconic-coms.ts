...
export default function (pi: ExtensionAPI) {
  pi.registerFlag("cname", {
    description: "Coms agent name. Distinct from pi --name, which the harness owns.",
    type: "string",
    default: undefined,
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    try {
      ...
      notify(ctx, `coms: bind failed. ${err instanceof Error ? err.message : String(err)}`, "error");
      return;
    }
  ...
  pi.registerTool({
    ...
    label: "Coms list",
    description:
      "List live peer Pi sessions on this machine (name, model, purpose, cwd, alive). Use for talking to another already-running session. Do not use for swarm, arena, or orchestrate. Those stay draconic_spawn.",
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      if (!peer) return toolText("coms not initialised", { ok: false });
      try {
        const sent = await peer.send({ target: params.target, prompt: params.prompt });
        ...
        return toolText(`coms_send failed: ${message}`, { ok: false, error: message });
      }
    },
  });
  ...
  pi.registerTool({
    name: "coms_get",
    label: "Coms get",
    description: "Non-blocking poll of a coms_send reply. Returns pending, complete, or error.",
    ...
    async execute(_toolCallId, params) {
      ...
      if (got.status === "error") {
        return toolText(`coms_get: error. ${got.error}`, { status: "error", error: got.error });
      }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      msg_id: Type.String({ description: "msg_id returned by coms_send." }),
      timeout_ms: Type.Optional(Type.Number({ description: "Override timeout in milliseconds." })),
    }),
    async execute(_toolCallId, params) {
      if (!peer) return toolText("coms not initialised", { ok: false });
      const timeoutMs =
