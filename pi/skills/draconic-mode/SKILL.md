...
**Start every multi-step task with a checklist in `.draconic/TODO.md` (or the `draconic_todo` tool) whose first item is to read the Principles section below in full.** The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
**Just do it.** Use bash, `gh`, and project CLIs. Pi has no built-in MCP. Reversible work and external actions proceed without asking.
...
Pi has no Task tool. When the `draconic_spawn` tool is loaded, use it for playbook delegates, arena arms, swarm workers, interrogate reviewers, and how/why explorers. Pass a standalone prompt with file paths, success criteria, and the prove-it-works bar. Set `readonly: true` for review-only children. Set `worktree: true` when the child writes. Pass `model` from `.pi/draconic-models.md` when a role has a real slug.

If spawn is missing, do the step in this session. Review your own diff before calling it done. A same-session self-review is allowed; pretending a missing child ran is not. Write `skip: no spawn runtime` on a playbook step only when the step's value was multi-model diversity and you cannot get it.
...
This pack runs on **Pi**. Map leftover Cursor or OpenCode wording as follows:
