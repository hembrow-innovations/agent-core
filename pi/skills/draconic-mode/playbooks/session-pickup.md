...
1. Locate the prior trail. A local Pi session under `~/.pi/agent/sessions/` for this cwd, `PI_SESSION_FILE`, `/resume` or `/tree`, or a pushed branch. Read the last messages first, then scan back for the decision points. Parse a long transcript with `draconic_spawn` or a summary file and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
